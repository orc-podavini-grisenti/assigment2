...
Path Tracking without Terminal Cost.

#               N-1
#   min          ∑   ( w_v*∥dq_k∥^2 + w_a∥u_k∥^2 + w_w∥w_k∥^2 )     
#  X,U,S,W      k=0
#               |---------------- Running Cost ---------------|        
#
...
def run_q1(visualize=True, plot_results=True):
    ...
    print("Minimizes velocity, torque, and path deviation without terminal cost")
    ...
    ocp = path_tracking(opti, X, U, S, W, N, dt,
                        robot_setup.x_init,
                        robot_setup.c_path,
                        robot_setup.r_path,
                        w_v,
...
# OCP Formulation for Path Tracking with Progress Variable s(t)
def path_tracking(opti, X, U, S, W, N, dt, x_init,
    ...
    for k in range(N):
        ...
        # - q_min <= q_k <= q_max
        # - dq_min <= dq_k <= dq_max
        # They are already included during variable definition in run_q1()
    

    # Terminal Step Costs and Constraints
